#!/bin/bash
# AXIS SCHOOL SYSTEM – ULTIMATE FIX
# - Adds URL‑based tenant middleware
# - Forces all missing tables
# - Generates fees & syncs data

set -e

echo "============================================"
echo "ULTIMATE PATCHER – FIXES ROUTING & TABLES"
echo "============================================"
...
# 2. Create the custom middleware file
echo "→ Creating URL‑based tenant middleware..."
mkdir -p axis_saas/middleware
cat > axis_saas/middleware/url_tenant_middleware.py << 'EOF'
...
EOF
echo "   Middleware file created."

# 3. Update settings.py to include the new middleware
echo "→ Updating settings.py to use URL‑based middleware..."
...
if match:
    old_middleware = match.group(1)
    # Insert our custom middleware before 'django_tenants.middleware.main.TenantMainMiddleware'
    new_middleware = re.sub(
        r"('django_tenants\.middleware\.main\.TenantMainMiddleware')",
        r"'axis_saas.middleware.url_tenant_middleware.URLPathTenantMiddleware',\n    \1",
        old_middleware
    )
    # Ensure our middleware is not already there
    if "URLPathTenantMiddleware" not in new_middleware:
        new_middleware = new_middleware.replace(
    ...
    print("   ⚠️ Could not find MIDDLEWARE list in settings.py – manual intervention needed.")
PYTHON_UPDATE_SETTINGS

# 4. Run shared migrations
echo "→ Migrating shared (public) schema..."
python manage.py migrate_schemas --shared

# 5. Run tenant migrations (standard)
echo "→ Running standard tenant migrations..."
python manage.py migrate_schemas --tenant

# 6. Force‑create any missing tables (schema_editor fallback)
echo "→ Force‑creating missing axis_saas tables in all tenant schemas..."
...
for tenant in tenants:
    ...
    with schema_context(tenant.schema_name):
        ...
        if not exists:
            print(f"   ⚠️ Student table missing – creating all axis_saas tables.")
            ...
            print(f"   ✅ Student table exists.")
        # Run any pending migrations for auth etc.
        call_command('migrate', interactive=False, verbosity=0)
PYTHON_FORCE_CREATE

# 7. Data fixes (custom_fee, fee generation, overdue status)
...
echo "============================================"
echo "ULTIMATE PATCHER FINISHED"
echo "============================================"
echo ""
echo "✅ What was done:"
echo "   • Added URL‑based tenant middleware (fixes /portal/xxx/ routing)"
echo "   • Forced creation of any missing tables"
...
echo ""
echo "👉 Then access any school via: http://localhost:8000/portal/<schema_name>/"
